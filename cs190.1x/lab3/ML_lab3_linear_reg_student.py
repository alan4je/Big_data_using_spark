...
def parsePoint(line):
    ...
    """
    s = line.split(',')
    return LabeledPoint(float(s[0]), [float(p) for p in s[1:]])
...
onlyLabels = parsedDataInit.map(lambda s: s.label)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda s: LabeledPoint((s.label - minYear), s.features))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    rmse = np.sqrt(labelsAndPreds.map(lambda s: squaredError(s[0], s[1])).mean())
    return rmse
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda s: (s.label, averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda s: (s.label, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda s: (s.label, averageTrainYear))
...
def gradientSummand(weights, lp):
    ...
    """
    return DenseVector((weights.dot(DenseVector(lp.features)) - lp.label) * lp.features)
...
def getLabeledPrediction(weights, observation):
    ...
    """
    return (observation.label, DenseVector(weights).dot(DenseVector(observation.features)))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda s: getLabeledPrediction(w, s))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = DenseVector(trainData.map(lambda lp: gradientSummand(w, lp)).reduce(lambda a,b: np.add(a,b)))
...
labelsAndPreds = parsedValData.map(lambda s: getLabeledPrediction(weightsLR0, s))
...
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations= numIters, step= alpha, 
                                           ...
                                           regParam= reg, regType= regType, intercept= True)
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda s: (s.label, firstModel.predict(s.features)))
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label, np.hstack((lp.features,[a*b for a in lp.features for b in lp.features])))
