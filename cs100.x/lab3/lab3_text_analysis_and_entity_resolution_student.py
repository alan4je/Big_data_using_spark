...
def tokenize(string):
    ...
    """
    return [x for x in re.split(split_regex, string.lower()) if x not in stopwords]
...
def countTokens(vendorRDD):
    ...
    vendorRDD.map(lambda s: (s[0], len(s[1]))).foreach(lambda (k,v): accum.add(v))
    return accum.value
...
def findBiggestRecord(vendorRDD):
    ...
    return vendorRDD.map(lambda s: (s[0], s[1],len(s[1]))).takeOrdered(1, lambda (k,v,c): -c)
...
def idfs(corpus):
    ...
    N = corpus.count()*1.0
    def f(x): return x
    uniqueTokens = corpus.map(lambda (k,v): (k,set(v))).flatMapValues(f).map(lambda s: s[1])
...
def dotprod(a, b):
    ...
    """
    ab = set(a.keys()) & set (b.keys())
    return sum([a[i]*b[i] for i in ab])
...
def norm(a):
    ...
    """
    return math.sqrt(sum(map(lambda x: x ** 2, a.values())))
...
def cossim(a, b):
    ...
    """
    return dotprod(a,b)/(norm(a) * norm(b)*1.0)
...
# TODO: Replace <FILL IN> with appropriate code
def f(x): return x
amazonList = amazonSmall.collect()
crossSmall = (googleSmall
              .map(lambda (k,v): ((k,v), amazonList)).flatMapValues(f)
              .cache())
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (k,v,sc): (v+' '+k, sc))

trueDupsRDD = (sims
               .join(goldStandard))

trueDupsCount = trueDupsRDD.count()
avgSimDups = sum(trueDupsRDD.map(lambda (k,v): v[0]).collect())/trueDupsCount

dupkeys = trueDupsRDD.map(lambda (k,v): k).collect()
nonDupsRDD = (sims
              .filter(lambda (k,v): k not in dupkeys))

avgSimNon = sum(nonDupsRDD.map(lambda (k,v): v).collect())/nonDupsRDD.count()
...
amazonTF = sc.broadcast(amazonFullRecToToken.map(lambda (k,v): (k, tf(v))).collectAsMap())
googleTF = sc.broadcast(googleFullRecToToken.map(lambda (k,v): (k, tf(v))).collectAsMap())
amazonWeightsRDD = amazonFullRecToToken.map(lambda (k,v): (k, dict([(i,idfsFullBroadcast.value[i]*amazonTF.value[k][i]) for i in v])))
googleWeightsRDD = googleFullRecToToken.map(lambda (k,v): (k, dict([(i,idfsFullBroadcast.value[i]*googleTF.value[k][i]) for i in v])))
...
def flatme(x): return x

amazonInvPairsRDD = (amazonWeightsRDD
                    .map(lambda s: (s[0],invert(s))).flatMapValues(flatme).map(lambda s: s[1])
                    .cache())
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda s: swap(s)).groupByKey()
                .cache())
