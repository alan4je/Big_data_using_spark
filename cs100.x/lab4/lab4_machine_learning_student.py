...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    noRatings = len(IDandRatingsTuple[1])
    avgRating = sum(IDandRatingsTuple[1])*1.0/noRatings
    return (key, (noRatings, avgRating))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda (uid, mid, r): (mid, r)).groupByKey())
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (mid, item): (item[1][1], item[0], item[1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda (uid, mid, r): ((uid, mid),r))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (uid, mid, r): ((uid, mid),r))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda (k,v): (v[0]-v[1])**2))
...
trainingAvgRating = trainingRDD.map(lambda (u,m,r): r).mean()
...
myRatedMovies = [(myUserID, 1088,5), (myUserID,1047,4), (myUserID,831,4.5), (myUserID, 1447, 4.8), (myUserID, 776, 4.3), 
                 (myUserID, 1250, 4.7), (myUserID, 811, 4), (myUserID, 860, 4.6), (myUserID, 1129, 4.6), (myUserID, 789, 5), 
                 (myUserID, 551, 5.8), (myUserID, 983, 4.9)
...
MYratedMovies = [i[1] for i in myRatedMovies]
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (k,v): k not in MYratedMovies).map(lambda (k,v): (0,k)))
...
predictedRDD = predictedRatingsRDD.map(lambda (k, m, r): (m, r))
...
ratingsWithNamesRDD = (predictedWithCountsRDD.filter(lambda (k,v): v[1] > 75)
                       .join(moviesRDD).map(lambda (mid,vals): (vals[0][0], vals[1], vals[0][1])))
