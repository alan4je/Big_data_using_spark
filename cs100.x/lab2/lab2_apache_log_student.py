...
dayToHostPairTuple = access_logs.map(lambda log: ((str(log.date_time).split('-')[2].split()[0],log.host),1)).reduceByKey(lambda a,b:a+b).map(lambda s: s[0])

dayGroupedHosts = dayToHostPairTuple.groupByKey()

dayHostCount = dayGroupedHosts.map(lambda dh: (int(dh[0]), len(dh[1])))
...
c = dailyHosts.collect()
dic = dict((str(k), v) for k, v in c)
print dic
...
dayAndHostTuple = access_logs.map(lambda log: (str(log.date_time).split('-')[2].split()[0],log.host))

groupedByDay = dayAndHostTuple.groupByKey().map(lambda d: (int(d[0]), len(d[1])))

sortedByDay = groupedByDay.sortByKey()
avgDailyReqPerHost = (sortedByDay.map(lambda s: (s[0], s[1]/dic[str(s[0])]))).cache()
...
badUniqueEndpoints = badEndpoints.map(lambda s: s[0])
...
errDateCountPairTuple = badRecords.map(lambda log: (str(log.date_time).split('-')[2].split()[0],1))

errDateSum = errDateCountPairTuple.reduceByKey(lambda a,b: a+b).map(lambda s:(int(s[0]), s[1]))
...
hourCountPairTuple = badRecords.map(lambda log: (int(str(log.date_time).split('-')[2].split()[1].split(':')[0]),1))
